...
public class Tester {
    ...
    public Tester(int[][] maze, int[] start, int[] destination, boolean expected) {
        this.maze = maze;
        this.start = start;
    ...
    @Test
    public void test() {
        boolean actual = solution.hasPath(maze, start, destination);
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("actual = {}, expected = {}", actual, expected);
        }
        assertThat(actual == expected, is(true));
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("method hasPath() pass unit test!");
        }
    }
