...
public class Tester {
    ...
    public Tester(int N, int K, int r, int c, double expected) {
        this.N = N;
        this.K = K;
        this.r = r;
        this.c = c;
    ...
    @Test
    public void test() {
        double actual = solution.knightProbability(N, K, r , c);
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("expected = {}, actual = {}", expected, actual);
        }
        assertThat(actual == expected, is(true));
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("method knightProbability() pass unit test!");
        }
    }
